...
G = nx.Graph()
depot = "depot"
junctions = [f"j{i}" for i in range(1, 6)]
...
orders_df = pd.DataFrame([ # Can this be in excel file?
    {"CustomerID": "p1", "ItemID": "A", "Amount": 23},
    {"CustomerID": "p1", "ItemID": "B", "Amount": 10},
    {"CustomerID": "p1", "ItemID": "H", "Amount": 12},
...
item_sizes = {"A":3,"B":2,...} # Can this be in excel file?
item_weight = {"A":2,"B":8,...} # Can this be in excel file?
...
for _, group in grouped_orders:
    ...
    for i, j in combinations(items, 2):
        co_occurrence[i][j] += 1
        co_occurrence[j][i] += 1
co_occurrence_pairs = {(i, j): co_occurrence[i][j] for i in co_occurrence for j in co_occurrence[i]}

# ---------------- Step 3: Compute LSC-based Placement ----------------
def compute_dynamic_pps(unplaced, placed, demand, weight, cooc, w_freq=0.8, w_cooc=0.2):
    freq_weight = {i: demand[i] * weight[i] for i in demand}
    max_freq_weight = max(freq_weight.values()) if freq_weight else 1
    total_cooc_all = {i: sum(cooc.get((i, j), 0) for j in demand if j != i) for i in demand}
    max_total_cooc = max(total_cooc_all.values()) if total_cooc_all else 1
    pps = {}
    for i in unplaced:
        freq_term = freq_weight[i] / max_freq_weight
        cooc_term = sum(cooc.get((i, j), 0) for j in placed) / max_total_cooc if placed else 0
        pps[i] = w_freq * freq_term + w_cooc * cooc_term
    return pps

def compute_lsc(item_id, b, item_weight, item_k, placed_blocks, cooc, w_depot=0.5, w_affinity=0.5):
...
def place_items_by_lsc(items, demand, weight, k_values, cooc):
    ...
    while sum(item_placed_count.values()) < sum(k_values.values()):
        ...
        best_block = min(
            available_blocks,
            key=lambda b: compute_lsc(current_item, b, weight, k_values, placed_blocks, cooc)
        )
...
# ---------------- Step 4: Evaluate ----------------
block_inventory = {block: block_capacity for block in block_assignment}
...
def get_sorted_blocks_for_item(item):
    return sorted(
        ...
        key=lambda b: (nx.shortest_path_length(G, depot, b, weight="weight"), block_inventory[b])
    )
...
print(total_distance, block_assignment)
